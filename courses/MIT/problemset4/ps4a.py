...
WORDLIST_FILENAME = "E:\work\code\MIT\problemset4\words.txt"
...
def getWordScore(word, n):
    ...
    for char in word:
        count = count + SCRABBLE_LETTER_VALUES[char]

    if wordLen != n:
        return count * wordLen
    else:
        return count * wordLen + 50
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while playFlag:
        ...
        if userWord == '.':
            print(f'Goodbye! Total score: {totalPoints} points.')
            playFlag = False
        else:
            if not isValidWord(userWord, hand, wordList):
                print('Invalid word, please try again.')
            else:
                wordPoints = getWordScore(userWord, HAND_SIZE)
                totalPoints += wordPoints
                print(f'"{userWord}" earned {wordPoints} points. Total: {totalPoints} points.')
                hand = updateHand(hand, userWord)
                if calculateHandlen(hand) == 0:
                    print(f'Run out of letters. Total score: {totalPoints} points.')
                    playFlag = False
